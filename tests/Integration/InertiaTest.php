...
final class InertiaTest extends TestCase
{
    ...
    public function test_location_response_for_inertia_requests(): void
    {
        ...
        static::assertSame(expected: Status::CONFLICT, actual: $response->status);
        static::assertSame(
    ...
    public function test_location_response_for_non_inertia_requests(): void
    {
        ...
        static::assertSame(expected: Status::FOUND, actual: $response->status);
        static::assertSame(
            expected: 'https://inertiajs.com',
    ...
    public function test_location_redirects_are_not_modified(): void
    {
        ...
        static::assertInstanceOf(Redirect::class, $response);
        static::assertSame(expected: Status::FOUND, actual: $response->status);
        static::assertSame(expected: '/foo', actual: $response->getHeader(name: 'Location')->values[0]);
    }
    ...
    {
        $redirect = new Redirect(to: 'https://inertiajs.com');
        $redirect->addSession(name: 'foo', value: 'bar');
        $response = $this->createFactory()->location($redirect);
        ...
        static::assertSame(expected: $response, actual: $redirect);
    }
    ...
    public function test_returns_response_if_it_isnt_an_inertia_request(): void
    {
        ...
        $response->assertOk();
        $response->assertHeaderContains(name: 'Vary', value: Header::INERTIA);
        static::assertSame(
            expected: [
    ...
    {
        get(Inertia::class)->share(key: 'foo', value: 'bar');

        static::assertArrayHasKey(key: 'foo', array: get(InertiaConfig::class)->sharedProps, message: 'bar');
        get(Inertia::class)->flushShared();

        static::assertSame(expected: [], actual: get(InertiaConfig::class)->sharedProps);
    }
